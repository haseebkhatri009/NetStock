...
import { useEffect, useMemo, useState } from 'react'
import { ref, push, onValue, remove, set } from 'firebase/database'
import { Plus, Search, Trash2, Building2, Phone, MapPin, X, User, FileSpreadsheet } from 'lucide-react'
import { db } from '../firebase'
import { useAuth } from '../context/AuthContext'
import { formatDate } from '../utils/helpers'
import Loader from '../components/Loader'
...
export default function Customers() {
  ...
  // Navigate to quotation page with customer ID
  function handleCreateQuotation(customerId) {
    navigate(`/quotation?customer=${customerId}`)
  }
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((c) => (
            <div key={c.id} className="bg-surface rounded-2xl border border-line shadow-card p-5">
              ...
              <button
                onClick={() => handleCreateQuotation(c.id)}
