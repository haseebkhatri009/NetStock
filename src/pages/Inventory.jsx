...
//   const [bulkShowDropdown, setBulkShowDropdown] = useState(false)
//   const [bulkIsCustomSelected, setBulkIsCustomSelected] = useState(false)
...
//     if (!bulkProductSearch.trim()) {
//       return PRODUCT_NAMES
//     }

//     const q = bulkProductSearch.toLowerCase().trim()
...
//     const macs = parseMultiValue(bulkForm.macs)
//     const serials = parseMultiValue(bulkForm.serials)

//     if (macs.length === 0) {
//       return {
//         count: 1,
//         qty: Math.max(1, Number(bulkForm.quantity) || 1),
//         mode: 'quantity'
//       }
//     }
...
//     }
//   }, [bulkForm.macs, bulkForm.serials, bulkForm.quantity])
...
//     if (bulkForm.productName && bulkForm.productName !== 'Custom') {
//       finalProductName = bulkForm.productName
//     } else if (bulkForm.customProductName.trim()) {
//       finalProductName = bulkForm.customProductName.trim()
//     }

//     const category =
//       bulkForm.category === 'Custom'
//         ? bulkForm.customCategory.trim()
//         : bulkForm.category

//     if (!category || !finalProductName) {
//       setBulkErrors(['Please select a product name and category'])
//       return
//     }

//     if (!bulkForm.addedDate) {
//       setBulkErrors(['Please select Date of Entry'])
//       return
//     }

//     const macsRaw = parseMultiValue(bulkForm.macs)
//     const serialsRaw = parseMultiValue(bulkForm.serials)

//     const macs = macsRaw.map((m) => normalizeMac(m)).filter(Boolean)
//     const serials = serialsRaw

//     if (macs.length > 0 && serials.length > 0 && macs.length !== serials.length) {
//       setBulkErrors([
//         `MAC count (${macs.length}) does not match Serial count (${serials.length}). Please provide equal numbers, or leave one side empty.`
//       ])
//       return
//     }
...
//     if (macs.length === 0) {
//       const qty = Math.max(1, Number(bulkForm.quantity) || 1)
...
//     macs.forEach((mac) => {
//       if (seen.has(mac)) {
//         errors.push(`Duplicate MAC inside batch: ${mac}`)
//       }
//       seen.add(mac)
//     })
...
//       if (exists) {
//         existingDupes.push(mac)
//       }
//     }

//     if (existingDupes.length > 0) {
//       errors.push(
//         `These MACs already exist in inventory: ${existingDupes.join(', ')}`
//       )
//     }
...
//         await push(stockRef, item)
//       }

//       setBulkForm({ ...emptyBulkForm, addedDate: getTodayDate() })
//       setBulkProductSearch('')
//       setBulkShowDropdown(false)
//       setBulkIsCustomSelected(false)
//       setBulkErrors([])
//       setShowBulkForm(false)

//       alert(
//         `Successfully added ${itemsToAdd.length} ${
//           itemsToAdd.length === 1 ? 'item' : 'items'
//         } to inventory.`
//       )
//     } catch (error) {
//       console.error('Bulk add failed:', error)
//       setBulkErrors(['Failed to add items. Please try again.'])
//     } finally {
//       setBulkSaving(false)
//     }
//   }
...
//   function bulkSelectProduct(name) {
...
//   /* ============================================================
//      BULK SELECT CUSTOM PRODUCT
//      ============================================================ */

//   function bulkSelectCustomProduct(
//     searchTerm
//   ) {
//     const customName =
//       searchTerm.trim() || 'Custom'
...
//     setBulkProductSearch(customName)
//     setBulkIsCustomSelected(true)
//     setBulkShowDropdown(false)
...
//   /* ============================================================
//      BULK PRODUCT INPUT CHANGE
//      ============================================================ */

//   function handleBulkInputChange(e) {
//     const value = e.target.value

//     setBulkProductSearch(value)

//     if (bulkIsCustomSelected) {
//       setBulkIsCustomSelected(false)
...
//     if (
//       bulkForm.productName &&
//       bulkForm.productName !== 'Custom'
//     ) {
//       setBulkForm({
//         ...bulkForm,
//         productName: '',
...
//             }}
//             className="flex items-center gap-2 rounded-lg bg-blue-600 text-white text-sm font-medium px-4 py-2.5 hover:bg-blue-700 transition-colors"
//           >
//             <ListPlus size={16} />
...
//                 <strong>Tip:</strong> Add multiple items at once.
//                 Paste MAC addresses and/or serial numbers — <strong>one per line</strong> (or comma separated).
//                 If no MAC is given, a single entry with the specified quantity will be created.
//               </p>
//             </div>
...
//             {/* ==================================================
//                 MAC ADDRESSES (MULTI-LINE)
//                 ================================================== */}

//             <label className="block">

//               <span className="text-xs font-medium text-slateink">
//                 MAC Addresses (one per line — optional)
//               </span>
...
//                 {parseMultiValue(bulkForm.macs).length} MAC(s) detected
//               </small>
...
//               <small className="text-xs text-slateink mt-1 block">
//                 {parseMultiValue(bulkForm.serials).length} Serial(s) detected
//               </small>
...
//                 placeholder="Common note for all items…"
//               />
...
//             <div className="bg-teal/10 border border-teal/30 rounded-lg p-3 text-sm">
//               <p className="font-medium text-teal-dark">
//                 Preview: {bulkPreview.count}{' '}
//                 {bulkPreview.count === 1 ? 'entry' : 'entries'} will be created
//                 {bulkPreview.mode === 'quantity' && (
//                   <> with quantity {bulkPreview.qty}</>
//                 )}
//                 {bulkPreview.mode === 'mac' && (
//                   <>
//                     {' '}
//                     (MACs: {bulkPreview.macCount}
//                     {bulkPreview.serialCount > 0
//                       ? `, Serials: ${bulkPreview.serialCount}`
//                       : ''}
//                     )
//                   </>
//                 )}
...
//             {bulkErrors.length > 0 && (
//               <div className="bg-coral-light border border-coral/30 rounded-lg p-3 space-y-1">
//                 {bulkErrors.map((err, i) => (
//                   <p key={i} className="text-xs font-medium text-coral">
//                     • {err}
//                   </p>
//                 ))}
//               </div>
...
//               type="submit"
//               disabled={bulkSaving}
//               className="w-full rounded-lg bg-blue-600 text-white text-sm font-medium py-2.5 hover:bg-blue-700 transition-colors disabled:opacity-60"
//             >
//               {bulkSaving
//                 ? 'Adding items…'
//                 : `Add ${bulkPreview.count} ${
//                     bulkPreview.count === 1 ? 'Item' : 'Items'
//                   } to Stock`}
//             </button>
...
export default function Inventory() {
  ...
  // ============================================================
  // MODEL-WISE GROUPING VIEW
  // ============================================================
  const [showModelWiseView, setShowModelWiseView] = useState(false)
  ...
  /* ============================================================
     MODEL-WISE GROUPED DATA
     ============================================================ */

  const modelWiseGroups = useMemo(() => {
    if (!stock) return []
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {/* MODEL WISE VIEW TOGGLE */}

          <button
            onClick={() =>
              setShowModelWiseView(!showModelWiseView)
            }
            className={`flex items-center gap-2 rounded-lg border text-sm font-medium px-4 py-2.5 transition-colors ${
              showModelWiseView
          ...
          >
            ...
            {showModelWiseView ? 'Model Wise View' : 'Model Wise View'}
          </button>
                ...
                <tbody>
                  ...
                  {modelWiseGroups
                    .filter((group) => {
                      const matchesCat =
                        activeCat === 'All' ||
                        group.categories.has(activeCat)
                    ...
                    .map((group, index) => {
                      ...
                      const isExpanded =
                        expandedModel === group.name

                      return (
                        <>
                          <tr
                            key={group.name}
                            className="border-b border-line hover:bg-paper/60 cursor-pointer"
                            onClick={() =>
                              setExpandedModel(
                                isExpanded ? null : group.name
                              )
                            }
                          >
                          ...
                          {/* EXPANDED ROWS - ALL INDIVIDUAL UNITS */}

                          {isExpanded && (
                            <tr key={`${group.name}-expanded`}>
                              <td colSpan={8} className="p-0 bg-paper/40">

                                <div className="px-6 py-4">

                                  <p className="text-xs font-semibold text-slateink uppercase tracking-wide mb-3">

                                    All Units — {group.name} ({group.items.length} {group.items.length === 1 ? 'entry' : 'entries'})
                                  ...
                                  <div className="overflow-x-auto">
                                    ...
                                    <table className="w-full text-xs">
                                      ...
                                      <thead>
                                        ...
                                        <tr className="border-b border-line text-left text-slateink">
                                      ...
                                      <tbody>
                                        ...
                                        {group.items.map((item, idx) => {
                                          ...
                                          const isItemDemo = item.status === 'demo'
                                          const isItemSold = item.status === 'sold'
                                          const canFullEdit = isOwner && !isItemDemo && !isItemSold
                                          const canEditDescription = isOwner && isItemSold
                                          ...
                                          return (
                                            ...
                                            >
                                              ...
                                              <td className="px-3 py-2 text-slateink">
                                                {idx + 1}
                                              </td>
                                              ...
                                              <td className="px-3 py-2 text-slateink font-mono">
                                                {formatDate(item.addedDate)}
                                              </td>

                                              <td className="px-3 py-2 text-slateink max-w-[180px] truncate">
                                                {item.description || '—'}
                                              </td>
                                                ...
                                                <div className="flex items-center gap-2 justify-center">
                                                  ...
                                                  {isItemDemo && (
                                                    <>
                                                      <button
                                                        onClick={() => {
                                                          const details = getDemoDetails(item)
                                                          setShowDemoDetails({
                                                            ...item,
                                                            demoTo: details.customerName,
                                                            demoCompany: details.customerCompany,
                                                            demoPhone: details.customerPhone
                                                          })
                                                        }}
                                                      ...
                                                      <button
                                                        onClick={() => handleReturnDemo(item.id)}
                                                        className="text-teal-dark hover:text-teal"
                                                        disabled={returning}
                                                        title="Return to stock"
                                                      >
